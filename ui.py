...
def show_all_tasks_frame(app):
    ...
    label = tk.Label(master=frame, text="Your Tasks", font=("Arial", 14, "bold"))
    label.grid(row=0, column=0, columnspan=3, pady=10)

    tasks = commands.get_tasks().to_list()
    for task in tasks:
        checkbtn = tk.Checkbutton(master=frame, text=task["title"])
        checkbtn.grid(row=tasks.index(task)+1, column=1, sticky="w", padx=5, pady=5)

        edit_btn = tk.Button(master=frame, text="Edit", command=partial(show_edit_task_frame, task, app))
        edit_btn.grid(row=tasks.index(task)+1, column=2, padx=5, pady=5)

        delete_btn = tk.Button(master=frame, text="Delete", command=partial(handle_delete, task["_id"], app))
        delete_btn.grid(row=tasks.index(task)+1, column=3, padx=5, pady=5)

    add_btn = tk.Button(master=frame, width=12, text="Add Task", command=lambda: show_add_task_frame(app))
    add_btn.grid(row=len(tasks) + 1, column=0, columnspan=3, pady=15)
